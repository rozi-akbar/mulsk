(function ($) {
  ...
  $.fn.kallesRefresh_flickity = function () {
    $(this).flickity(
      JSON.parse(
        $(this).attr("data-flickity") || $(this).attr("data-flickityjs") || "{}"
      )
    );
    return $(this);
  };
  ...
  $.fn.closeMenu = function () {
    ...
    $(".nt_fk_canvas.act_opened,.nt_fk_full.act_opened").removeClass(
      "act_opened"
    );
    ...
    $(this).each(function () {
      ...
      if ($id.hasClass("nt_loop")) {
        ...
        for (i = 0; i < l; i++) {
          if (
            parseInt(timej[i], 10) >
            parseInt(shopt.toString().slice(0, timej[i].length), 10)
          ) {
            /*fix 24:00:00 error ios,safari*/
  ...
  $.fn.initCountdown = function () {
    $(this).each(function () {
      const _this = $(this);
      _this
        .countdown(_this.data("date"), { elapse: true })
        .on("update.countdown", function (event) {
          if (event.elapsed) {
  ...
  $.fn.initSeCountdown = function () {
    $(this).each(function () {
      const $this = $(this);
      $this
        .countdown($this.data("date"), function (event) {
          const template = [];
          template.push(
            '<span class="countdown-days">%-D <span>days</span></span>'
          );
          template.push(
            '<span class="countdown-hours">%H <span>hr</span></span>'
          );
          template.push(
            '<span class="countdown-min">%M <span>min</span></span>'
          );
          template.push(
            '<span class="countdown-sec">%S <span>sc</span></span>'
          );
    ...
    if ($(window).width() > 767) {
      $(this).each(function () {
        let li = $(this),
          ...
          bodyRight =
            global_wrapper.outerWidth() + global_wrapper.offset().left,
          viewportWidth =
        ...
        let dropdownOffsetRight =
          screenWidth - nav_dropdownOffset.left - nav_dropdownWidth;

        if (nav_dropdownWidth >= 0 && li.hasClass("menu_center")) {
          let toLeft =
            nav_dropdownOffset.left + nav_dropdownWidth / 2 - screenWidth / 2;
          if (rtl_mode) {
          ...
          (dropdownOffsetRight + nav_dropdownWidth >= viewportWidth ||
            li.hasClass("menu_wid_full")) &&
          li.hasClass("menu_has_offsets")
        ) {
          var toLeft = dropdownOffsetRight + nav_dropdownWidth - viewportWidth;
          nav_dropdown.css({ right: -toLeft - extraSpace });
        } else if (
          (nav_dropdownOffset.left + nav_dropdownWidth >= viewportWidth ||
            li.hasClass("menu_wid_full")) &&
          li.hasClass("menu_has_offsets")
        ) {
          var toRight =
  ...
  $.fn.initStickyMenu = function () {
    if (
      body.hasClass("header_sticky_false") ||
      (body.hasClass("des_header_7") && window_w > 1400)
    )
      return;
    let $header = $("#ntheader"),
      timeout = 0,
  ...
  $.fn.kallesSwatches = function () {
    if ($(this).length) {
      body.on("click", ".swatch_pr_item:not(.is-selected)", function (evt) {
        ...
        if ($this.data("escape")) {
          $this
            .closest(".kalles_swatch_js")
            .find(".user_choose_js,.nt_name_current")
            .text($this.data("escape"));
        }
        if ($this.data("index")) {
          const $main_slide = $this
            .closest(".kalles-section")
            .find(".col_thumb .pr_carousel")
            .first();
          if ($main_slide) {
        ...
        if ($this.data("filter")) {
          ...
          if ($p_thumb.length && Array.isArray(options)) {
            ...
            options.forEach((p) => {
              thumb_nav.push(
                `<div class="${p.class}" data-mdid="${p.mdid}" data-height="${p.height}" data-width="${p.width}" data-ratio="${p.ratio}" data-mdtype="${p.mdtype}" data-src="${p.src}" data-bgset="${p.bgset}" data-cap="${p.cap}"></div>`
              );
            });
            ...
            $p_thumb
              .flickity("destroy")
              .empty()
              .append(thumb_nav.join(""))
              .kallesRefresh_flickity();
            ...
            setTimeout(
              () =>
                $(
                  ".kalles_product_thumnb_slide"
                ).kallesSingleProductThumbnail(),
              100
            );
            ...
            $(".pr_sticky_content").kallesStickySingleProductDetail(
              !is_smaller_768 && body.hasClass("single-product-template")
            );
        ...
        let popup = $(this).closest(".popup_age_wrap"),
          stt =
            popup && popup.data("stt")
              ? popup.data("stt")
              : { day_next: 30, date_of_birth: true, age_limit: 18 },
  ...
  $.fn.kallesDisableNavSlider = function () {
    if ($(this).length) {
      $(this).each(function () {
        ...
        if (attrs) {
          const selectedCells = attrs.selectedCells
            ? attrs.selectedCells.length
            : null;
    ...
    if ($(this).length) {
      ...
      $(this).each(function () {
        let $el = $(this).find(".p-nav"),
          ...
          $col_nav = $(".col_nav"),
          timer = 400,
          includedModelOrVideos =
            $mainGallery.find("[data-pr-md-tp-model]").length > 0 ||
            $mainGallery.find("[data-pr-md-tp-video]").length > 0;

        $mainGallery.each(function (index) {
          initThumbnailsMarkup(
            $(this),
            $el.length === 1 ? $el : $el.eq(index),
            navClass
          );
        });
        ...
        if (includedModelOrVideos) {
          $mainGallery.on("select.flickity", function (event, index) {
            ...
            $(
              ".p-thumb [data-pr-md-tp-video] , .p-thumb [data-pr-md-tp-model]"
            ).trigger("mediaHidden");
            ...
            if (["video", "external_video", "model"].includes(mediaType)) {
              if (mediaType === "model") {
                ...
                $selectedSlide
                  .find("[data-pr-md-tp-model]")
                  .trigger("mediaVisible");
              } else {
                $p_btns.addClass("nt_hide");
                $selectedSlide
                  .find("[data-pr-md-tp-video]")
                  .trigger("mediaVisible");
              }
            }
          });
        ...
        $col_nav
          ...
          .on("click", function (e) {
            ...
            if (
              ($this.hasClass("btn_pnav_prev") && currentScrollTop <= 0) ||
              ($this.hasClass("btn_pnav_next") &&
                currentScrollTop >= slider_height - navHeight)
            )
              return;
            let scrollY = $this.hasClass("btn_pnav_next")
              ? currentScrollTop + navCellHeight + 40
              : currentScrollTop - navCellHeight - 40;
            $el.stop().animate(
              {
                scrollTop: scrollY,
      ...
      const createPlayer = function (media) {
        ...
        if (media.host === "html5") {
          ...
          media.element.addEventListener("load", function () {
            media.container
              .find(".kalles-model-viewer-ui__button--poster")
              .addClass("d-none");
            media.container
              .find(".kalles-model-viewer-ui__controls-area")
              .addClass("kalles-model-viewer-ui__controls-area--playing");
            $(this)
              .find("#userInput")
          ...
          media.container.on(
            "click",
            ".kalles-model-viewer-ui__button--poster",
            function (ev) {
              ev.preventDefault();
              media.element.setAttribute(
                "src",
                media.element.getAttribute("data-src")
              );
            }
          );
          ...
          media.container.on(
            "click",
            ".kalles-model-viewer-ui__button--fullscreen",
            function (evt) {
              evt.preventDefault();
              evt.stopPropagation();
              if (
                $(this).attr("modelFullScreen") &&
                  ...
                  $wrapModel[0]
                    ...
                    .then(() =>
                      setTimeout(
                        () =>
                          $wrapModel.addClass(
                            "kalles-model-viewer-ui--fullscreen"
                          ),
                        100
                      )
                    );
                } else if ($wrapModel[0].webkitRequestFullscreen) {
                  /* Safari */
                  $wrapModel[0]
                ...
                } else if ($wrapModel[0].msRequestFullscreen) {
                  /* IE11 */
                  $wrapModel[0]
                    .msRequestFullscreen()
                    .then(() =>
        ...
        media.container.on("mediaHidden xrLaunch", function () {
          if (media.host === "html5" && media.player) {
            ...
            if (
              _data.hasOwnProperty("draggable") &&
              _data["draggable"] === ">1"
            ) {
              /*Enable dragging*/
              updateDraggableFlickity($carousel.data("flickity"), ">1");
    ...
    if ($(this).length) {
      ...
      let getProductItems = function (getvl) {
        ...
        img_visible.each(function () {
          ...
          _html +=
            '<div class="pswp_thumb_item"><img class="lazyload" src="data:image/svg+xml,%3Csvg%20viewBox%3D%220%200%20' +
            width +
            "%20" +
            height +
            '%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3C%2Fsvg%3E" data-src="' +
            img.replace("1x1", img_url) +
            '" data-widths="[50, 100]" data-sizes="auto" alt=""></div>';
        });
      ...
      let callPhotoSwipe = function (index, items) {
        ...
        let options = {
          ...
          bgOpacity: 1,
          showHideOpacity:
            $mainImages.hasClass("nt_contain") ||
            $mainImages.hasClass("nt_cover"),
      ...
      $(this).on("click", ".show_btn_pr_gallery", function (e) {
        ...
        let items = getProductItems(),
          ...
          index = $mainImages.hasClass("isotope_ok")
            ? $(this).parents().index()
            : getCurrentGalleryIndex(e);
        if (index === -1) {
          index = 0;
        }
      ...
      if ($productGallery.hasClass("img_action_zoom") && window_w > 1025) {
        ...
        if (
          zoom_tpl === 2 &&
          ($(".thumb_2").length > 0 || $(".thumb_3").length > 0)
        ) {
          zoom_tpl = 1;
  ...
  $.fn.kallesDropdownPicker = function () {
    if ($(this).length) {
      $(this).on(
        "click",
        ".header_picker, .kalles_dropDown_label",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
          ...
          if ($(pr).hasClass("opended")) {
            ...
            $(pr.closest(".dropdown_picker_js"))
              .find(".nt_lt_fake.opended ,.kalles_dropdown_container.opended")
              .removeClass("opended");
            $(pr).addClass("opended");
          }
        }
      ...
      body.on("click", function (e) {
        if (
          $(e.target).hasClass("header_picker") ||
          $(e.target).hasClass("kalles_dropDown_label")
        )
          return;
        $(".nt_lt_fake.opended,.kalles_dropdown_container.opended").removeClass(
          "opended"
        );
      });
      ...
      body.on("click", ".kalles_dropdown_option", function (e) {
        ...
        if ($this.data("label")) {
          $this
            .closest(".kalles_dropdown_container")
            .find(".kalles_dropDown_label .lbl-title")
  ...
  $.fn.kallesRefreshPriceTitle = function () {
    if ($(this).length) {
      ...
      $(this).each(function () {
        ...
        _amount
          .find("input.url_price, input.min_price, input.max_price")
          .hide();
  ...
  $.fn.kallesDiscountPromotionPopup = function () {
    ...
    if (
      ...
      Cookies.get("kalles_prpr_pp") === "shown"
    ) {
    } else {
        ...
        $.magnificPopup.open({
          ...
          callbacks: {
            ...
            open: function () {
              ...
              $wrap_popup
                .find(".pr_coun_dt:not(.done_cd) , .sepr_coun_dt:not(.done_cd)")
                .initCountdown();
  ...
  $.fn.kallesLoadKemenper = function () {
    const $btn = $(this),
      $container = $("#KEMENTRIAN-PERINDUSTRIAN"),
      data = $container.length ? $container.html() : null;
  ...
  $.fn.kallesLoadQuikShop = function () {
    ...
    if (data.length) {
      $.magnificPopup.open({
        items: {
          src:
            '<div class="mfp-with-anim pp_qs" id="content_quickview">' +
            data +
            "</div>" /* can be a HTML string, jQuery_T4NT object, or CSS selector*/,
          type: "inline",
        },
        tClose: "Close (Esc)",
        removalDelay: 500 /*delay removal by X to allow out-animation*/,
  ...
  if (window_w >= 1024) {
    ...
    body.on("click", function (e) {
      let target = $(e.target);
      if (
        target.closest(".pin__type").length > 0 ||
        target.closest(".mfp-wrap").length > 0
      )
        return;
      $(".pin__type.pin__opened").removeClass("pin__opened");
      $(".pin__slider.pin_slider_opened").removeClass("pin_slider_opened");
  ...
  body.on("click", "[data-opennt]", function (e) {
    var $this = $(e.currentTarget),
      ...
      height =
        window.height -
        $("#kalles-section-header_banner").outerHeight() -
  ...
  body.on(
    "click",
    "button.plus , a.plus , button.minus , a.minus",
    function (e) {
      e.preventDefault();
      e.stopPropagation();
      const $inputTag = $(this)
        .closest(".quantity")
        .find(".qty_cart_js,.qty_pr_js");
      if ($inputTag) {
  ...
  if ($(".r--reply-form-wrapper").length) {
    ...
    body.on("click", ".btn_control_reply_comment", function (e) {
      e.preventDefault();
      e.stopPropagation();
      return $(this).hasClass("reply-comment--on")
        ? open_comment_form($(this).closest(".r--reply-form-wrapper"))
        : close_comment_form($(this).closest(".r--reply-form-wrapper"));
    });
  }
    ...
    if ($window.width() > 1024) {
      ...
      if (html.hasClass("browser-Safari")) {
        const footerSafariFix = function () {
          ...
          if (
            footerOffsetTop <
            windowScroll + $footer.outerHeight() + $window.outerHeight()
          ) {
   ...
   * ********************************************/
  if (
    body.hasClass("cart_pos_dropdown") &&
    !touchevents &&
    ...
    let $cart = $("#nt_cart_canvas"),
      ...
      right =
        windowWidth - ($icon_cart.offset().left + $icon_cart.outerWidth()),
  ...
  $(
    ".kalles-section-pr_summary .single_add_to_cart_button , .sticky_atc_btn .single_add_to_cart_button"
  ).kallesAnimation();
  ...
  setTimeout(
    () => $(".kalles_product_thumnb_slide").kallesSingleProductThumbnail(),
    100
  );
