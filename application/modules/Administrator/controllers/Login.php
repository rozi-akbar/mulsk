...
class Login extends Authentication
{
    ...
    public function AuthLogin()
    {
        ...
        if ($get_status['status'] == 1) {
            $this->session->set_userdata('isLogin_mulsk', $get_status['username']);
            $this->session->set_userdata('id_user_mulsk', $get_status['id']);
            $this->session->set_userdata('username_mulsk', $get_status['username']);
            $this->session->set_userdata('nama_user_mulsk', $get_status['nama']);
            $this->session->set_userdata('level_mulsk', $get_status['level']);
            $this->session->set_userdata('nama_level_mulsk', $get_status['nama_level']);
